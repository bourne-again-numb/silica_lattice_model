...
i_teos="24";     	d_teos="1";     	f_teos=${i_teos}
i_taaoh="3";     	d_taaoh="1";     	f_taaoh=${i_taaoh}
i_h2o="9500";  	        d_h2o="1";   	        f_h2o=${i_h2o}
manual_concen_flag="0"
...
#testing variables
i_size="50";     	d_size="10";       	f_size=${i_size}
nsweeps="10000";nprint="50";nsnapshot="100";neqsweeps="300"
time_limit="0.0014"
#-------------------------
...
for sisn in $(seq ${i_sisn} ${d_sisn} ${f_sisn});do # SISNinteraction
	...
	for nteos in $(seq ${i_teos} ${d_teos} ${f_teos});do #SIconcentration
		...
		for nh2o in $(seq ${i_h2o} ${d_h2o} ${f_h2o});do #TAAconcentration
		
		    for t in $(seq ${i_t} ${d_t} ${f_t});do #temperature
		    
			for cu in $(seq ${i_size} ${d_size} ${f_size});do #system size
			        	            
			    CURRENT_VERSION=${VERSION}_SISN${sisn}_SITAA${sitaa}_nTEOS${nteos}_nTAAOH${ntaaoh}_nH2O${nh2o}_tstar${t}_${cu}cu
			    mkdir -p ${CURRENT_VERSION}

			    #enter the current version directory
			    cd ${CURRENT_VERSION}
			    for j in $(seq 1 1 ${RUNS}) #run no.
			    do
				...
				echo "${nteos},${ntaaoh},${nh2o},${manual_concen_flag}" >> ${INPUT}
